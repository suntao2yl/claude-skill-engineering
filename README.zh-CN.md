...
本 skill **把 implementation 相位委托给** [`harness-plan`](https://github.com/suntao2yl/claude-skill-harness) skill。没有 harness-plan，相位 1-3 和 5-7 能跑，但相位 4（implementation）会卡住。
...
- `~/.codex/skills/harness-plan/SKILL.md`，或
...
先安装 `harness-plan`，再安装 `harness-engineering`：
...
安装完成后请重启 Codex，这两个 skill 才会出现在技能列表中。
